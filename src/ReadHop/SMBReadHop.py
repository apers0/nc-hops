...
class ExtractHopProcessing:
    ...
    def detect_encoding(self) -> str | None:
        ...
        """
        with smbclient.open_file(self.path_to_file, "rb") as hfile:
            raw_data = hfile.read()
    ...
    def open_file(self) -> list:
        ...
        with smbclient.open_file(self.path_to_file, "r", encoding=encoding) as hfile:
        ...
        for i, line in enumerate(contents):
            ...
            if stripped_line == "START":  # Target lines are after START
                ...
                for processing_steps in contents[i:]:  # Target lines - contents[i:]
                    ...
                    if (
                        step.startswith("fertigteil")
                        or step.startswith("call hh_park")
                        or step.startswith("call hh_meaquote")
                        or step.startswith("call park_v7")
                    ):  # Various lines that are not relevant
                        continue  # continue looping if the condition is met
                    processing.append(
                        processing_steps.strip()
        ...
        for i, line in enumerate(contents):
            ...
            if stripped_line == "VARS":  # If the line is VARS
                ...
                for var_line in contents[i:]:  # contents[i:] are the target lines
                    if (
                        var_line.strip() == "START"
                    ):  # Stop when we get to the START string
                        break
                    split_string = var_line.strip().split(
                        ":="
                    )  # Strip and get rid of whitespace
                    var_value = split_string[-1].split(";")[0]  # Value of the variable
                    var_name = split_string[0]  # Name of the variable
                    var.update(
                        {var_name[0:-1]: var_value[1:]}
                    )  # Update the initialized dictionary with keys and values
        return var
    ...
    def vertical_drills(self) -> list:
        ...
        for process in processing:
            if str(process).lower().startswith("bohrung"):
                ...
                converted_values = [
                    float(value) if "." in value else int(value) for value in values
                ]
    ...
    def sawing(self) -> list:
        ...
        for process in processing:
            if str(process).lower().startswith("saegen"):  # Look for sawing macro
                ...
                values = coordinates.strip("()").split(
                    ","
                )  # split into a list and strip () from str
                converted_values = [
                    float(value) if "." in value else int(value) for value in values
                ]  # str to number
        ...
        for line in content:
            ...
            if line.lower().startswith("wzf"):
                ...
                if current_tool_no not in tools_dict:
                    tools_dict[current_tool_no] = {
                        "tool_no": current_tool_no,
                        "milling": [],
                    }
